...
import com.github.shyiko.mysql.binlog.gtidprofilingpoc.originalwithoutstring.OriginalGtidSetButWithoutString;
 ...
 */
public class DumpBinaryLogGtidCommandForNewGtidSet implements Command {

    private long serverId;
    private String binlogFilename;
    private long binlogPosition;
    private MySqlGtidSet gtidSet;

    public DumpBinaryLogGtidCommandForNewGtidSet(long serverId, String binlogFilename, long binlogPosition, MySqlGtidSet gtidSet) {
    ...
    public byte[] toByteArray() throws IOException {
        ...
        Collection<OriginalGtidSetButWithoutString.UUIDSet> uuidSets = Collections.emptySet();
        int dataSize = 8 /* number of uuidSets */;
        for (OriginalGtidSetButWithoutString.UUIDSet uuidSet : uuidSets) {
        ...
        for (OriginalGtidSetButWithoutString.UUIDSet uuidSet : uuidSets) {
            buffer.write(hexToByteArray(uuidSet.getUUID().toString().replace("-", "")));
            Collection<OriginalGtidSetButWithoutString.Interval> intervals = uuidSet.getIntervals();
            buffer.writeLong(intervals.size(), 8);
            for (OriginalGtidSetButWithoutString.Interval interval : intervals) {
